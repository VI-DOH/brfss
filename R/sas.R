...
sas.process.year<-function(year) {
...
read.xpt<-function(year,readfolder_pat=sas.folder.pattern.raw_data(),
  ...
  if(file.exists(xptname)) {
    ...
    savename<- sub_pattern(save_pat,year)
...
split.states<-function(year, loadfolder_pat=sas.folder.pattern.data(),
                       load_pat=sas.file.pattern.rdata()) {

  fname<-paste0("df_xpt_",year)

  if(!exists(fname))df_xpt<-load.sas(year,loadfolder_pat,load_pat)
  ...
  mapply(function(id,nm) {
    if(id%in%states) {
      ...
      fname<-paste0("df_",nm)
      assign(fname,df_state)
      save(list = c(fname),file = paste0("./data/",year,"/brfss_",nm,"_",year,".RData"))
      #browser()
